...
# Requiere: Flask, python-docx, Pillow
...
def build_default_bloques_from_payload(payload):
    ...
    for clave_map, readable in mapa.items():
        ...
        for idx in range(cantidad):
            ...
            push(f"FOTO DE {readable} {idx+1} - Código: {safe_str(codigo,'-')}", f"foto_9_acc_{clave_map}_{idx+1}", 1, True if len(lista)>0 else False)

    push("FOTO DE ZONA MEDIDORES", "foto_9_zona_medidores", 1, True)
    push("11. EVIDENCIA FOTOGRÁFICA DE LA INSTALACIÓN", "foto_11_1", 1, True)

    # PUNTO 10: Trabajos realizados (dinámicos)
    actividades_obj = actividades or {}
    for idx, t in enumerate(tanks):
        i = idx+1
        serie = safe_str(t.get("serie"))
        key_tank_candidates = [f"tank{i}", f"tank_{i}", f"tanque{i}", f"tanque_{i}"]
        actividades_tank = None
        for k in key_tank_candidates:
            if k in actividades_obj:
                actividades_tank = actividades_obj[k]
                break
        if not actividades_tank:
            # si en payload hay sección general "actividades" por índice numérico
    ...
    # trabajos en redes
    redes_works = ["red_llenado_retorno", "red_consumo"]
    for rw in redes_works:
        acts_rw = actividades_obj.get(rw) or actividades_obj.get(rw.replace("red_",""), [])
...
def generate_docx():
    ...
    # Tanques / accesorios / redes / equipos / observaciones (similar al anterior)
    ...
    if equipos:
        ...
        for eq in equipos:
            doc.add_paragraph(", ".join([f"{k}: {v}" for k, v in eq.items() if v]))
    ...
    # Bloques fotografícos (insertar imágenes)
    bloques = build_default_bloques_from_payload(payload)
    doc.add_page_break()
    doc.add_heading("Evidencias fotográficas", level=2)

    # Normalizar keys y aplanar posibles claves __n
    flattened = {}
    ...
    # Detalle de actividades (10.X) - ANTES / DESPUÉS agrupado
    doc.add_page_break()
    ...
    if actividades_obj:
        for key, acts in actividades_obj.items():
            ...
            for a_idx, act in enumerate(acts, start=1):
                ...
                # claves esperadas
                before_keys = [f"foto_10_{key}_act{a_idx}__antes", f"foto_10_{key}_act{a_idx}__1", f"foto_10_{key}__antes"]
                after_keys = [f"foto_10_{key}_act{a_idx}__despues", f"foto_10_{key}_act{a_idx}__2", f"foto_10_{key}__despues"]
                before_files = []
                after_files = []
                ...
                hdr = tbl.rows[0].cells
                hdr[0].text = "ANTES"; hdr[1].text = "DESPUÉS"
                row = tbl.add_row().cells
                if before_files:
