...
def generar_docx_desde_dfs(
    ...
    # --- Toma desplazada (grupo): 
    # Show the 3 general subtítulos (punto transferencia, caja, recorrido) only if at least one of the toma types exists.
    toma_llenado_list = accesorios_red_for_block.get("llenado_toma_desplazada", [])
    toma_retorno_list = accesorios_red_for_block.get("retorno_toma_desplazada", [])
    toma_group_exists = bool(toma_llenado_list or toma_retorno_list)
    ...
    # Accesorios individuales (mapa) - for valves: enable only those actually present (per-key)
    mapa = {
    ...
    for clave, nombre in mapa.items():
        ...
        # only mark 'existe' True for those that actually are present (so frontend shows valve rows only when user added that valve)
        for idx in range(cantidad):
            if idx < len(lista):
                ...
                codigo = "-"
                existe = False
            # tokens: allow both descriptive token and numeric 9_N token; also include clave and specific indexed clave token
    ...
    # - luego red_llenado y red_consumo si aplican
    sec_idx = 1
    ...
    # 3) Red de llenado (si hay actividades con contexto 'red_llenado')
    acts_llenado = actividades_por_contexto.get("red_llenado", [])
    if acts_llenado:
    ...
    # 4) Red de consumo (si hay actividades con contexto 'red_consumo')
    acts_consumo = actividades_por_contexto.get("red_consumo", [])
    if acts_consumo:
    ...
    # NOTA: No incluimos secciones de 'general' aquí por petición.
